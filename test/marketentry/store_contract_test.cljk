...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X SRL" :jurisdiction "DOM"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0
                                   :mipyme-reserved-award? true :women-led-claim? true
                                   :micm-certification-valid? true :micm-certification-women-led? true
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "DOM" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:micm-certification-women-led? (:engagement m)) (:micm-certification-women-led? (:engagement d))))
