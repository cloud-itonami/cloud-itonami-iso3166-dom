  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Malecón Ingeniería SRL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Malecón Ingeniería SRL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest mipyme-reserved-ineligible-is-held-and-unoverridable
  (testing "declared :mipyme-reserved-award? true but certification/fiscal-standing facts fail eligibility -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:mipyme-reserved-ineligible} (-> (store/ledger db) last :basis)))
...
(deftest rnc-record-unverified-is-held-and-unoverridable
  (testing "unverified RNC tax record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:rnc-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
