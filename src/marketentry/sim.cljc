...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (DOM, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up mipyme-reserved-ineligible) ==")
    ...
    (println "== filing/submit eng-4 (mipyme-reserved-ineligible -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up rnc-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (rnc-record-unverified -> HARD hold) ==")
